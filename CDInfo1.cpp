...
void CCDInfo::DrawOffScreen(HDC hdc, RECT rc)
{
	...
	// First make sure we have the coordinates in device units and we are offset from 0,0
...
{
	// REVIEW: Also check height & take minimum
...
LRESULT CCDInfo::OnLButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	POINT	pt;
	short	nTrack;
...
LRESULT CCDInfo::OnMouseMove(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
short CCDInfo::GetTrackFromPoint(const POINT& pt)
{
	POINT	ptCenter;
	short   nTracks, nTrack, nTotalLength;
	double  dblRadius = (m_rcPos.right - m_rcPos.left) / 2;
...
void CCDInfo::DrawToTrack(short nTrack)
{
	...
	// REVIEW: Perhaps we should just invalidate ourselves
	if (m_bWndLess)
	{
...
void CCDInfo::CreateTooltipWindow()
{
	...
	InitCommonControls(); 
//	m_wndToolTip.CreateEx(NULL, rect, TTS_ALWAYSTIP);
	m_wndToolTip.m_hWnd = 
...
// been created if we are window or if we are windowless the HWND of the client 
